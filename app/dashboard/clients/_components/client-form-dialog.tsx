  ...
  return (
    <DialogRoot open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="bg-background sm:max-w-lg p-10">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold tracking-[-0.04em]">{isEditing ? "Editar cliente" : "Novo cliente"}</DialogTitle>
          <DialogDescription className="px-0">{isEditing ? "Atualize os dados deste cliente." : "Cadastre um cliente para começar a organizar sua carteira."}</DialogDescription>
        </DialogHeader>

        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="space-y-2">
            <Label htmlFor="client-name">Nome</Label>
            <Input id="client-name" name="name" required value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} placeholder="Nome completo" className="h-11 rounded-md" />
          </div>

          <div className="space-y-2">
            <Label htmlFor="client-contact">Contato</Label>
            <Input id="client-contact" name="contact" required value={form.contact} onChange={(event) => setForm((current) => ({ ...current, contact: event.target.value }))} placeholder="E-mail ou telefone" className="h-11 rounded-md" />
          </div>
          ...
          <DialogFooter className="mt-6 border-t-0 bg-transparent p-0">
            ...
            <Button type="submit" disabled={isSaving}>{isSaving ? "Salvando..." : isEditing ? "Salvar alterações" : "Cadastrar cliente"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </DialogRoot>
